...
class CategoriaActivo(AutoCodeMixin, BaseModel):
    """
...
class EstadoActivo(BaseModel):
    ...
    permite_movimiento = models.BooleanField(
        default=True, verbose_name="Permite Movimiento"
    )
...
class Proveniencia(AutoCodeMixin, BaseModel):
    """
...
class Taller(AutoCodeMixin, BaseModel):
    """
...
class Activo(BaseModel):
    ...
    def _generar_codigo(self) -> str:
        ...
        ultimo_activo = (
            Activo.objects.filter(categoria=self.categoria, eliminado=False)
            .exclude(pk=self.pk)
            .order_by("-codigo")
            .first()
        )
    ...
    def save(self, *args: Any, **kwargs: Any) -> None:
        ...
        if not self.codigo_barras and self.codigo:
            codigo_limpio: str = (
                self.codigo.replace("-", "").replace("_", "").upper()[:12]
            )
...
class MovimientoActivo(BaseModel):
    ...
    def __str__(self) -> str:
        ...
        ubicacion: str = (
            self.ubicacion_destino.nombre if self.ubicacion_destino else "Sin ubicación"
        )
        responsable: str = (
            self.responsable.get_full_name() if self.responsable else "Sin responsable"
        )
