...
class OrdenCompraForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        solicitudes_disponibles = Solicitud.objects.filter(
            estado__codigo="COMPRAR", eliminado=False
        ...
        self.fields["solicitudes"].queryset = solicitudes_disponibles.order_by(
            "-numero"
        )
        ...
        self.fields[
            ...
            "Seleccione las solicitudes en estado COMPRAR que desea asociar (opcional)"
        )

        # Establecer estado inicial como PENDIENTE al crear
        if not self.instance.pk:
            try:
                estado_pendiente = EstadoOrdenCompra.objects.get(
                    codigo="PENDIENTE", activo=True
                )
                self.fields["estado"].initial = estado_pendiente
            except EstadoOrdenCompra.DoesNotExist:
                # Fallback: primer estado activo
                estado_inicial = (
                    EstadoOrdenCompra.objects.filter(activo=True)
                    .order_by("codigo")
                    .first()
                )
                if estado_inicial:
...
class DetalleOrdenCompraActivoForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["activo"].queryset = (
            Activo.objects.filter(activo=True, eliminado=False)
            .select_related("categoria")
            .order_by("codigo")
        )
...
class OrdenCompraFiltroForm(forms.Form):
    ...
    q = forms.CharField(
        ...
        widget=forms.TextInput(
            attrs={"class": "form-control", "placeholder": "Buscar por número..."}
        ),
    )
    ...
    proveedor = forms.ModelChoiceField(
        required=False,
        label="Proveedor",
        queryset=Proveedor.objects.filter(activo=True, eliminado=False).order_by(
            "razon_social"
        ),
        widget=forms.Select(attrs={"class": "form-select"}),
    )
...
class RecepcionArticuloForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            ...
            "orden_compra": forms.Select(
                attrs={"class": "form-select", "id": "id_orden_compra"}
            ),
            "bodega": forms.Select(attrs={"class": "form-select"}),
            "documento_referencia": forms.TextInput(
                attrs={"class": "form-control", "placeholder": "Guía/Factura"}
            ),
            "observaciones": forms.Textarea(attrs={"class": "form-control", "rows": 3}),
        }
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["orden_compra"].queryset = (
            OrdenCompra.objects.exclude(
...
class RecepcionArticuloFiltroForm(forms.Form):
    ...
    estado = forms.ModelChoiceField(
        required=False,
        label="Estado",
        queryset=EstadoRecepcion.objects.filter(activo=True, eliminado=False).order_by(
            "nombre"
        ),
        widget=forms.Select(attrs={"class": "form-select"}),
    )
...
class TipoRecepcionForm(forms.ModelForm):
    ...
    El código se genera automáticamente con prefijo TRC.
    """

    class Meta:
        model = TipoRecepcion
        fields = ["nombre", "descripcion", "requiere_orden", "activo"]
        widgets = {
            "nombre": forms.TextInput(
                attrs={
                    "class": "form-control",
            ...
            "requiere_orden": forms.CheckboxInput(attrs={"class": "form-check-input"}),
            "activo": forms.CheckboxInput(attrs={"class": "form-check-input"}),
        }
...
class EstadoOrdenCompraForm(forms.ModelForm):
    ...
    def clean_codigo(self) -> str:
        ...
        if queryset.exists():
            raise ValidationError(
                f'Ya existe un estado de orden de compra con el código "{codigo}".'
            )
